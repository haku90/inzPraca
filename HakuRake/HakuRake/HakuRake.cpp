...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	path1.set_length(38400);
	path2.set_length(38400);
	path3.set_length(38400);
	path4.set_length(38400);
	path5.set_length(38400);
	path6.set_length(38400);

	cvec radioFrameScrambling,radioFrameDescr,radioFrameChannel1,radioFrameChannel2;
	cmat coffeChannel;
	tdlChanel.init();
	...
	for(int i=0;i<EbN0dB.length();i++)
	{
		...
		for(int j=0;j<numOfIteration;j++)
		{
			...
			//kanal
			awg.set_noise(N0(i));
			radioFrameChannel1=awg(radioFrameScrambling);
			...
			coffeChannel=conj(coffeChannel);
				...
				for(int i=0;i<38400;i++)
				{
					mulPath1(i)=path1(i)*coffeChannel(i,0);
					mulPath2(i)=path2(i)*coffeChannel(i,1);
					mulPath3(i)=path3(i)*coffeChannel(i,2);
					mulPath4(i)=path4(i)*coffeChannel(i,3);
					mulPath5(i)=path5(i)*coffeChannel(i,4);
					mulPath6(i)=path6(i)*coffeChannel(i,5);

				
				}
					for(int i=0;i<38400;i++)
				{
					sum(i)=mulPath1(i)+mulPath2(i)+mulPath3(i)+mulPath4(i)+mulPath5(i)+mulPath6(i);

				
				}

			radioFrameDescr=rec.descrambling(sum,tr.gold.cConjugate);
